...
function Creatnow() {
  ...
  const [lastDesignBlob, setLastDesignBlob] = useState(null); // store last design
  ...
  // ── Merge transparent design onto hoodie ──
  const mergeWithHoodie = (transparentBlob, hoodieColor) => {
  ...
  const generateImage = async (e) => {
    ...
    setImageUrl("");
    setAdded(false);
    setLastDesignBlob(null);

    try {
      // Step 1: Generate image from backend
      setLoadingMsg("Generating design...");
      ...
      // Step 2: Remove background in browser
      setLoadingMsg("Removing background...");
      const transparentBlob = await removeBackground(blob);

      // Step 3: Store blob for color switching later
      setLastDesignBlob(transparentBlob);

      // Step 4: Merge onto hoodie
      setLoadingMsg("Placing on hoodie...");
      const mergedUrl = await mergeWithHoodie(transparentBlob, selectedColor);
  ...
  // Color change reuses last design — no new API call
  const handleColorChange = async (color) => {
    setSelectedColor(color);
    if (lastDesignBlob) {
      setLoading(true);
      setLoadingMsg("Switching color...");
      const mergedUrl = await mergeWithHoodie(lastDesignBlob, color);
      setImageUrl(mergedUrl);
      setLoading(false);
    }
  };
          ...
          {loading && (
            <div className="absolute inset-0 flex items-center justify-center bg-black/30 rounded-3xl">
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-white text-sm font-medium tracking-widest uppercase">{loadingMsg}</p>
              </div>
            </div>
          )}
        </div>
