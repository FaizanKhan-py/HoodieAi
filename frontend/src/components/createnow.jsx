...
function Creatnow() {
  ...
  // Re-merge with new hoodie color (reuses last design — no new API call)
  const handleColorChange = async (color) => {
    setSelectedColor(color);
    if (imageUrl) {
      setLoading(true);
      // Re-merge existing design with new hoodie color
      const mergedUrl = await mergeWithHoodie(imageUrl, color);
      setImageUrl(mergedUrl);
      setLoading(false);
    }
  };
